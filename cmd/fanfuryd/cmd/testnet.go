...
Example:
	fanfuryd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			genParams := TestnetGenesisParams()

			return InitTestnet(
				clientCtx, cmd, config, mbm, genBalIterator, genParams, outputDir, chainID, minGasPrices,
	...
	cmd.Flags().String(flagNodeDaemonHome, "fanfuryd", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagStartingIPAddress, "192.168.0.1", "Starting IP address (192.168.0.1 results in persistent peers list ID0@192.168.0.1:46656, ID1@192.168.0.2:46656, ...)")
	cmd.Flags().String(flags.FlagChainID, "", "genesis file chain-id, if left blank will be randomly created")
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", appparams.DefaultBondDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001stake)")
...
func InitTestnet(
	...
	for i := 0; i < numValidators; i++ {
		...
		coins := sdk.Coins{
			...
			sdk.NewCoin(appparams.DefaultBondDenom, accStakingTokens),
		}
