...
// Setup initializes a new FanfuryApp
func Setup(isCheckTx bool) *FanfuryApp {
	db := dbm.NewMemDB()
	app := NewFanfuryApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, 5, MakeEncodingConfig(), simapp.EmptyAppOptions{})
	if !isCheckTx {
...
func setup(withGenesis bool, invCheckPeriod uint) (*FanfuryApp, GenesisState) {
	...
	app := NewFanfuryApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, invCheckPeriod, encCdc, EmptyAppOptions{})
	if withGenesis {
