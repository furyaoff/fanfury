...
TEST_DOCKER_REPO=furyaoff/fanfurydnode
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=go-fanfury \
...
build: go.sum
ifeq ($(OS),Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o build/fanfuryd.exe ./cmd/fanfuryd
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/fanfuryd ./cmd/fanfuryd
endif
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/fanfuryd
...
build-docker-go-fanfury:
	$(MAKE) -C contrib/localnet

# Run a 4-node testnet locally
localnet-start: build-linux build-docker-fanfurydnode
	@if ! [ -f build/node0/fanfuryd/config/genesis.json ]; \
		then docker run --rm -v $(CURDIR)/build:/fanfuryd:Z fanfury/fanfury:song testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; \
	fi
	docker-compose up -d
...
.PHONY: all build-linux install install-debug \
	...
	build-docker-fanfurydnode localnet-start localnet-stop test-docker test-docker-push \
